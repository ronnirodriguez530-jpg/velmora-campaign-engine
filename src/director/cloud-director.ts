...
type SubmittedPlan = {
  ...
  questGenerations: Array<{ sourceThreadId: string; relationships: Array<{ questId: string; type: "prerequisite" | "parallel" | "optional_branch" | "consequence" }>; reason: string }>;
  questRecoveries: Array<{ failedQuestId: string; recoveryPath: string; consequenceEventSequences: number[]; reason: string }>;
  questDirectionRevisions: Array<{ questId: string; invalidatedDirectionId: string; consequenceEventSequences: number[]; reason: string }>;
  questUpdates: Array<{
    questId: string;
    ...
    newObjectiveFamily: "discover" | "influence" | "secure" | "change" | null;
    newObjectiveIsMajor: boolean | null;
    outcomeId: string | null;
    resolutionSummary: string | null;
    resolutionAreas: Array<"problem" | "people_or_factions" | "location_or_world" | "player_reward_or_cost">;
  ...
  parameters: {
    ...
    properties: {
          ...
          properties: {
            ...
            relationships: {
              ...
              items: {
                ...
                properties: {
                  questId: { type: "string" },
                  type: { type: "string", enum: ["prerequisite", "parallel", "optional_branch", "consequence"] }
                },
                required: ["questId", "type"],
                additionalProperties: false
              }
            },
            reason: { type: "string" }
          },
          required: ["sourceThreadId", "relationships", "reason"],
          additionalProperties: false
        }
      },
      questRecoveries: {
        type: "array",
        maxItems: 2,
        description: "Create up to two simultaneously pursuable altered routes from a recoverably failed supplied quest using distinct exact recorded recovery paths, subject to the source thread's hard two-unresolved-quest cap.",
        items: {
          type: "object",
          properties: {
            failedQuestId: { type: "string" },
            recoveryPath: { type: "string" },
            consequenceEventSequences: { type: "array", items: { type: "integer", minimum: 1 }, minItems: 1, maxItems: 4 },
            reason: { type: "string" }
          },
          required: ["failedQuestId", "recoveryPath", "consequenceEventSequences", "reason"],
          additionalProperties: false
        }
      },
      questDirectionRevisions: {
        type: "array",
        maxItems: 3,
        description: "Remove one unchosen direction only when cited durable consequences make it impossible. The engine alone decides whether a credible replacement exists.",
        items: {
          type: "object",
          properties: {
            questId: { type: "string" },
            invalidatedDirectionId: { type: "string" },
            consequenceEventSequences: { type: "array", items: { type: "integer", minimum: 1 }, minItems: 1, maxItems: 4 },
            reason: { type: "string" }
          },
          required: ["questId", "invalidatedDirectionId", "consequenceEventSequences", "reason"],
          additionalProperties: false
        }
      },
      questUpdates: {
        type: "array",
        maxItems: 3,
        description: "Manage supplied quests only when justified. Add adaptive objectives only when play creates genuinely new work. Record the exact causal resolution at completion; an unexpected valid solution may complete the underlying problem without checking off a presumed route.",
        items: {
          type: "object",
          properties: {
            questId: { type: "string" },
            action: { type: "string", enum: ["make_available", "activate", "add_objective", "complete_objective", "fail_objective", "complete", "complete_unexpectedly", "fail_recoverably", "fail_from_consequence", "record_warning", "apply_neglect_complication"] },
            objectiveId: { anyOf: [{ type: "string" }, { type: "null" }] },
            newObjectiveSummary: { anyOf: [{ type: "string" }, { type: "null" }] },
            newObjectiveFamily: { anyOf: [{ type: "string", enum: ["discover", "influence", "secure", "change"] }, { type: "null" }] },
            newObjectiveIsMajor: { anyOf: [{ type: "boolean" }, { type: "null" }] },
            outcomeId: { anyOf: [{ type: "string" }, { type: "null" }] },
            resolutionSummary: { anyOf: [{ type: "string" }, { type: "null" }] },
            resolutionAreas: { type: "array", items: { type: "string", enum: ["problem", "people_or_factions", "location_or_world", "player_reward_or_cost"] }, maxItems: 4 },
            consequenceEventSequences: { type: "array", items: { type: "integer", minimum: 1 }, maxItems: 4 },
            warningMethod: { anyOf: [{ type: "string", enum: ["directly_witnessed_or_clearly_told", "established_npc_message", "obvious_environmental_warning"] }, { type: "null" }] },
            warningSignal: { anyOf: [{ type: "string" }, { type: "null" }] },
            warningSourceNpcId: { anyOf: [{ type: "string" }, { type: "null" }] },
            neglectTrigger: { anyOf: [{ type: "string", enum: ["ignored_warning_after_deliberate_choice", "recorded_world_event_advances_threat"] }, { type: "null" }] },
            neglectComplicationTool: { anyOf: [{ type: "string", enum: ["change_faction_condition", "change_npc_reputation", "record_location_consequence", "manage_npc_turn", "manage_story_thread"] }, { type: "null" }] },
            reason: { type: "string" }
          },
          required: ["questId", "action", "objectiveId", "newObjectiveSummary", "newObjectiveFamily", "newObjectiveIsMajor", "outcomeId", "resolutionSummary", "resolutionAreas", "consequenceEventSequences", "warningMethod", "warningSignal", "warningSourceNpcId", "neglectTrigger", "neglectComplicationTool", "reason"],
          additionalProperties: false
        }
      },
      suggestedActions: {
...
Use questGenerations only to ask the engine to build a quest from an active supplied story thread; do not write the quest yourself. A quest's classification is permanently inherited from its origin thread; never reclassify it when its later role changes. Generate at most one ordinary quest in a turn. The second unresolved slot is optional, never a quota. An alternative must differ through its approach and tradeoff, allies or location, or moral or resource cost. If validation reports that no credible alternative can be constructed, omit that generation request and explain the specific established in-world obstacle naturally; never invent an obstacle merely to refuse the player. Describe follow-up causality with explicit prerequisite, parallel, optional_branch, or consequence relationships justified by the referenced quest's current state. Never close another unresolved route merely because one route completed. Use questDirectionRevisions only on an available uncommitted quest when supplied durable consequence events make one exact offered direction impossible. Cite the related events and explain the causal obstacle. Do not propose replacement content: the engine independently adds at most one unused causally credible direction and otherwise leaves fewer choices. Never revise a chosen route; invalidate that quest through fail_from_consequence when its committed premise becomes impossible. Use fail_from_consequence only when supplied durable consequence events make its premise impossible; cite those exact events and preserve the reason in the journal. Use questRecoveries only after a recoverable quest has failed and durable world consequences have been recorded, selecting up to two distinct exact supplied recovery paths and citing the exact consequence-event sequences. The two-new-quest exception is recovery-only: never mix ordinary and recovery quest generation in one turn. Both altered routes may remain pursuable, but the source thread may never exceed two simultaneous unresolved quests. Recovery may appear immediately after the consequence-bearing failure turn or later; it may not appear before evidence exists. Recovery changes the route while preserving the failed quest as history, its source thread, secrecy, and stage ceiling. Record a quest warning only when the player directly witnesses or is clearly told it, receives it from an established NPC, or encounters an obvious environmental warning; use an exact stored warning signal. Quest neglect requires either that received warning followed by the player's deliberate choice of another priority or a recorded world event advancing the threat. Each neglect request must cite new evidence and pair with exactly one matching bounded faction, NPC, location, or story-thread complication using the same reason. Repeated neglect remains mild without separately verified exceptional stakes: never turn it into automatic permanent failure, stack multiple complications in one turn, or impose an excessive punishment. Use at most three total quest updates or direction revisions in one turn and only for transitions justified by the current action. Resolve one active objective at a time. Add an objective only when new information, a world change, or the player's chosen approach creates genuinely new work; use discover, influence, secure, or change, never pad a quest, and continue through a linked quest instead of exceeding five objectives. Quests may mix social, investigation, exploration, and combat, and normally preserve a credible noncombat route unless established circumstances truly require combat. If an unexpected player solution actually resolves the underlying problem, use complete_unexpectedly rather than forcing a listed route. For any completion, write the exact resolution from what occurred and identify the affected problem, people or factions, location or world, and player reward or cost as applicable. Outcome consequences must be submitted through the ordinary validated faction, NPC, location, or story-thread tools in the same plan so the engine commits the outcome and its effects atomically. Ordinary generated quests are recoverable. Fail recoverably only when the supplied quest records a recovery path; never invent permanent failure authority without verified warned-deadline, irreversible-choice, or major-world-event stakes.
...
Call for a check only when the outcome is both uncertain and meaningful. Do not roll for ordinary movement, conversation, observation of obvious facts, harmless choices, or impossible actions. An impossible action is automatic only in the sense that no roll occurs; explain the specific established in-world reason it cannot achieve the stated result. Do not invent an obstacle solely to block a creative action.
...
Write an evocative but focused opening in 2-4 short paragraphs. Establish what the character perceives, what is happening now, and why a response matters. When opening.phase is exploration, begin from opening.spawn.spawnArea and opening.spawn.entryReason, allow unhurried exploration and interaction, and do not begin or imply the First Speaker strike. If visibleOpeningPressure is present later, make that crisis the immediate playable situation without exposing hidden blueprint material. You may create temporary sensory detail and dialogue, but may not invent permanent lore, powers, mechanics, hidden truths, new factions, or off-screen knowledge.
...
function parseSubmittedPlan(value: unknown): DirectorTurnPlan {
  ...
  if (plan.questGenerations.length > 1) throw new Error("Cloud Director may normally generate at most one quest per turn");
  if (plan.questRecoveries.length > 2) throw new Error("Cloud Director may generate at most two recovery quests per turn");
  if (plan.questGenerations.length > 0 && plan.questRecoveries.length > 0) throw new Error("Cloud Director cannot mix ordinary and recovery quest generation in one turn");
  if (plan.questUpdates.length + plan.questDirectionRevisions.length > 3) throw new Error("Cloud Director may create at most three quest-state changes per turn");
  ...
  const toolRequests: ToolRequest[] = [
    ...
    ...plan.questDirectionRevisions.map((change) => ({ type: "revise_quest_directions" as const, ...change })),
